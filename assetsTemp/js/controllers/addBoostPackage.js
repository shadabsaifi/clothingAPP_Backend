'use strict';
app.controller('addBoostPackageCtrl', ['$scope', '$localStorage', 'tapNCultureService', 'toastr','$state', function($scope, $localStorage, tapNCultureService, toastr,$state) {
    ...
    $scope.savePackage = () =>{
        ...
        tapNCultureService.addNewBoost($scope.obj).then((response) => {
            console.log(JSON.stringify(response));
            if (response.responseCode === 200) {
                toastr.success("Package Saved!","Success")
                $scope.obj = {};
                $scope.obj.packageImage =""
                // $scope.addPremiumAccount.$setPristine(true);//comment by me
                // console.log($scope.$$phase)
                // if($scope.$$phase){
                //     console.log($digest);
                //     console.log($apply)
                // }
                $state.go('header.boostList',{},{reload:'header.boostList'});
            }
        }, (err) => {
            console.log(err)
        })
