'use strict';
app.controller('boostListCtrl', ['$scope', '$state', 'tapNCultureService', 'toastr', '$localStorage', function($scope, $state, tapNCultureService, toastr, $localStorage) {
    ...
    let reuseFn = () => {
        ...
        console.log(obj)
        tapNCultureService.showBoostList(obj).then((response) => {
            ...
            if (response.responseCode === 200) {
                ...
                console.log("=====>response.data"+JSON.stringify(response.data.docs))
                console.log("=====>response.data"+JSON.stringify(response.data.docs.length))
    ...
    $scope.validation1 = () => {
        ...
        //  alert("ng-change",$scope.packageSearch)
    ...
    $scope.finalDelete = () => {
        ...
        tapNCultureService.deleteBoost(pck).then((response) => {
            // console.log(response)
            if (response.responseCode === 200) {
                $scope.currentPage = 1;
                obj.filterWord = '';
                reuseFn();
                toastr.success('Boost Deleted', "Success")
            }
            // else{
            //     toastr.error("Error due to bad network", "Error")
            // }
    ...
    let reuseFn2 = () => {        
        ...
        tapNCultureService.getBoostDetail(objSubs, $scope.currentPageSubs).then((response) => {
    ...
    $scope.addBoostPackage = () => {
        console.log("hi")
        $state.go('header.addBoostPackage')
